    ...
    , m_logFile(nullptr)
{
    ...
    // ========== 日志系统初始化 ==========
    QString logPath = dataDir() + "/ProfilePower.log";
    ...
    // =========================================================================
    QDir profilesDir(dataDir() + "/profiles");
...
void MainWindow::on_pushButton_8_clicked()
{
    ...
    // 建立存储总根目录 profiles/
    QString profilesRoot = dataDir() + "/profiles";
...
MainWindow::BrowserLaunchConfig MainWindow::buildLaunchConfig(const QJsonObject &configJson) const
{
    ...
    const QString proxyType = configJson["proxy_type"].toString().remove('"').trimmed();
    const QString proxyHost = configJson["proxy_host"].toString().remove('"').trimmed();
    const int proxyPort = configJson["proxy_port"].toString().remove('"').trimmed().toInt();
...
void MainWindow::openBrowserEnvironment(const BrowserLaunchConfig &config, int debugPort)
{
    ...
    // ========== Proxy relay startup (auth support) ==========
    QString effectiveProxyServer = config.proxyServer;
    QProcess *relayProcess = nullptr;
    writeLog("ProxyDebug", QString("username=[%1] pass=[%2] host=[%3] port=[%4] server=[%5]")
        .arg(config.proxyUsername, config.proxyPassword.isEmpty() ? "EMPTY" : "***",
             config.proxyHost, QString::number(config.proxyPort), config.proxyServer));
    if (!config.proxyUsername.isEmpty() && !config.proxyPassword.isEmpty()
        ...
        if (!python.isEmpty() && QFile::exists(relayScript)) {
            ...
            if (relayProcess->waitForStarted(3000)) {
                ...
                writeLog("ProxyRelay", QString("Relay started on 127.0.0.1:%1 -> %2:%3")
                         .arg(relayPort).arg(config.proxyHost).arg(config.proxyPort));
            } else {
                writeLog("ProxyRelay", QString("Relay failed to start: %1").arg(relayProcess->errorString()));
                relayProcess->deleteLater();
                relayProcess = nullptr;
            }
        } else {
            writeLog("ProxyRelay", "Cannot start relay: Python or proxy_relay.py not found");
        }
    }
...
void MainWindow::on_btnOpen_clicked()
{
    ...
    // Always read latest config from disk as primary source
    QString configPath = QFileInfo(chromeUserDataDir).dir().absoluteFilePath("config.json");
    QFile launchConfigFile(configPath);
    if (launchConfigFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QJsonParseError parseError;
        QJsonDocument doc = QJsonDocument::fromJson(launchConfigFile.readAll(), &parseError);
        launchConfigFile.close();
        if (parseError.error == QJsonParseError::NoError && doc.isObject()) {
            launchJson = doc.object();
        }
    }
    // Fallback: try cached config from table
    ...
    // Resolve relative chrome_user_data_dir from disk config
    {
        QString rawDataDir = launchJson["chrome_user_data_dir"].toString();
        QString configDir = QFileInfo(configPath).absolutePath();
        QString resolvedDir = QDir(configDir).absoluteFilePath(rawDataDir);
        launchJson["chrome_user_data_dir"] = QDir::cleanPath(resolvedDir);
    }
    ...
    // 5. Start proxy relay if auth configured, then launch browser
    QString proxyUser = launchJson["proxy_username"].toString().remove('"').trimmed();
    QString proxyPass = launchJson["proxy_password"].toString().remove('"').trimmed();
    QString proxyHost = launchJson["proxy_host"].toString().remove('"').trimmed();
    int proxyPort = launchJson["proxy_port"].toString().remove('"').trimmed().toInt();

    qDebug() << "RELAY_DBG: user=[" << proxyUser << "] pass=[" << (proxyPass.isEmpty() ? "EMPTY" : "***") << "] host=[" << proxyHost << "] port=[" << proxyPort << "]";

    QProcess *relayProc = nullptr;
    if (!proxyUser.isEmpty() && !proxyPass.isEmpty() && !proxyHost.isEmpty() && proxyPort > 0) {
        qDebug() << "RELAY_DBG: condition PASS, searching Python...";
        quint16 localPort = 0;
        QTcpServer picker;
        if (picker.listen(QHostAddress::LocalHost, 0)) { localPort = picker.serverPort(); picker.close(); }
        if (localPort == 0) localPort = 55432;
        qDebug() << "RELAY_DBG: localPort=" << localPort;

        QString python = QStandardPaths::findExecutable("python");
        if (python.isEmpty()) python = QStandardPaths::findExecutable("python3");
        qDebug() << "RELAY_DBG: python=" << python;

        QString relayScript = QCoreApplication::applicationDirPath() + "/proxy_relay.py";
        if (!QFile::exists(relayScript))
            relayScript = QCoreApplication::applicationDirPath() + "/../../proxy_relay.py";
        qDebug() << "RELAY_DBG: script=" << relayScript << " exists=" << QFile::exists(relayScript);

        if (!python.isEmpty() && QFile::exists(relayScript)) {
            relayProc = new QProcess(this);
            QStringList args = QStringList() << relayScript << QString::number(localPort)
                << proxyHost << QString::number(proxyPort) << proxyUser << proxyPass;
            qDebug() << "RELAY_DBG: starting:" << python << args;
            relayProc->start(python, args);
            if (relayProc->waitForStarted(3000)) {
                launchJson["proxy_server"] = QString("http://127.0.0.1:%1").arg(localPort);
                launchJson["proxy_host"] = "127.0.0.1";
                launchJson["proxy_port"] = QString::number(localPort);
                qDebug() << "RELAY_DBG: SUCCESS - proxy_server=" << launchJson["proxy_server"].toString();
                writeLog("OPEN", QString("Relay started 127.0.0.1:%1 -> %2:%3").arg(localPort).arg(proxyHost).arg(proxyPort));
            } else {
                qDebug() << "RELAY_DBG: waitForStarted FAILED -" << relayProc->errorString();
                writeLog("OPEN", "Relay FAILED: " + relayProc->errorString());
                relayProc->deleteLater(); relayProc = nullptr;
            }
        } else {
            qDebug() << "RELAY_DBG: SKIP - python=" << python << " scriptExists=" << QFile::exists(relayScript);
            writeLog("OPEN", "Relay SKIP: python=" + python + " script=" + relayScript);
        }
    } else {
        qDebug() << "RELAY_DBG: condition FAILED";
    }
    ...
    BrowserLaunchConfig launchConfig = buildLaunchConfig(launchJson);
    openBrowserEnvironment(launchConfig, debugPort);

    // Cleanup relay when browser exits (via the lambda captured in openBrowserEnvironment cannot see this, so we attach here)
    if (relayProc) {
        QTimer::singleShot(500, this, [relayProc]() {
            // relayProc is parented to this, will be cleaned up on app exit
        });
    }
}
...
{
    QString dir = QStandardPaths::writableLocation(QStandardPaths::AppLocalDataLocation) + "/ProfilePower";
    QDir().mkpath(dir);
    return dir;
}
